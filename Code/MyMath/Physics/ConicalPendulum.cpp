...
void ConicalPendulum::Update(){
	angularVelocity = std::sqrtf(9.8f / (length * std::cosf(halfApexAngle)));
	angle += angularVelocity * delTime;

	float radius = sinf(halfApexAngle) * length;
	float height = cosf(halfApexAngle) * length;

	ballPos = anchor + Vec3(cosf(angle) * radius,-height,-sinf(angle) * radius);
}

void ConicalPendulum::Draw(const MyMatrix4x4 &viewProjMat,const MyMatrix4x4 &viewPortMat,uint32_t color){
